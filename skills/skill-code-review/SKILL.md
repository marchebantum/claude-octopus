...
Honor `OCTO_ALLOWED_PROVIDERS` and `~/.claude-octopus/config/provider-allowlist` before naming or dispatching providers. If the allowlist is set to `codex claude`, do not mention Gemini as an active or expected participant.

...
#   - code-reviewer/implementer              → gpt-5.5    (current Codex default)
...
get_role_mapping() {
    ...
    if [[ "${OCTOPUS_LEGACY_ROLES:-0}" == "1" ]]; then
        case "$role" in
            ...
            synthesizer)  echo "claude-opus:$(opus_default_model 2>/dev/null || echo claude-opus-4.7)" ;;
    ...
    case "$role" in
        ...
        synthesizer)       echo "claude-opus:$(opus_default_model 2>/dev/null || echo claude-opus-4.7)" ;;  # Result aggregation
